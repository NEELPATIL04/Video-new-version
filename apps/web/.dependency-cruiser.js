...
module.exports = {
  forbidden: [
    {
      // DEV_STANDARDS.md §3.1: app/ is routing only. Mirrors the
      // eslint-plugin-boundaries rule in eslint.config.mjs — this is the
      // CI-level backup gate if that ever gets bypassed.
      name: 'features-no-import-app',
      severity: 'error',
      comment:
        'features/lib/components must not import from app/ (routing-only).',
      from: { path: '^src/(features|lib|components)' },
      to: { path: '^src/app' },
    },
    {
      name: 'no-circular',
...
// generated: dependency-cruiser@18.2.0 on 2026-09-08T19:35:58.983Z
