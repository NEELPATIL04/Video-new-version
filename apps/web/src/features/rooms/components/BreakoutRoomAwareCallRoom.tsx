...
interface BreakoutRoomAwareCallRoomProps {
  roomId: string;
  // Same seed-value contract as CallRoom's own initialRole — see that
  // component's comment. isHost below is derived from this, not passed
  // in directly, so a co-host promotion/demotion that happens while this
  // participant is already in a breakout room stays consistent with
  // whatever CallRoom itself resolves for the main connection.
...
export function BreakoutRoomAwareCallRoom({
  ...
  // Unlike CallRoom's own canManage/isHost, this doesn't need live
  // tracking: the true host's role can never change mid-call (only a
  // participant can be promoted/demoted to/from co-host), and
  // BreakoutRoomsController's host-only actions are strict RoomHostGuard
  // — a co-host is never exempt from breakout assignment either way.
