...
module.exports = {
  forbidden: [
    {
      // DEV_STANDARDS.md §3 / CLAUDE.md: Repository pattern, mechanically
      // enforced as a CI-level hard gate (mirrors the ESLint no-restricted-imports
      // rule in .eslintrc.js — this is the backup gate if that ever gets bypassed).
      name: 'controllers-no-direct-prisma',
      severity: 'error',
      comment:
        'Controllers must not import Prisma directly — go through a service/repository.',
    ...
    },
    {
      ...
      comment:
        ...
        'NOTE: downgraded from error to warn — enhanced-resolve currently misresolves some scoped ' +
        '@nestjs/* packages through pnpm\'s symlinked store on Windows even though they install and ' +
        'run fine (verified via `node -e "require.resolve(...)"`). Revisit if a real unresolvable ' +
        'import needs catching; for now this would only produce false positives on every run.',
      severity: 'warn',
      from: {},
      to: {
  ...
  options: {
    ...
    enhancedResolveOptions: {
      ...
      // Packages without a "main" field (e.g. @nestjs/common) rely on
      // Node's implicit index.js fallback — enhanced-resolve needs this
      // spelled out explicitly, it isn't a default.
      mainFiles: ["index"],
...
// generated: dependency-cruiser@18.2.0 on 2026-09-08T19:30:08.263Z
